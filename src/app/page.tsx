...
export default function DashboardPage() {
  ...
  const enablePushNotifications = async () => {
    if ('Notification' in window) {
      ...
      if (permission === 'granted') {
        ...
        new Notification('🎯 Surge Hunter Driver Alerts Enabled', {
    ...
    try {
      ...
      if (data.platformSurges) {
        ...
        if (notificationsEnabled && 'Notification' in window && Notification.permission === 'granted') {
          ...
          if (criticalSpot) {
            new Notification(`🚨 High Earnings Surge Alert: ${criticalSpot.zoneName}`, {
              body: `Surge Score reached ${criticalSpot.score}/100. High driver demand active.`,
              icon: 'https://cdnjs.cloudflare.com/ajax/libs/twemoji/14.0.2/72x72/1f3af.png',
            });
          }
        }
  ...
  return (
    <main className="min-h-screen bg-[#070A12] text-slate-100 flex flex-col font-sans antialiased selection:bg-emerald-500 selection:text-white">
      <AnalyticsModal 
      ...
      <div className="bg-[#0D1322]/80 backdrop-blur-md border-b border-slate-800/80 px-4 py-2 text-xs font-medium flex items-center justify-between text-slate-300">
        <div className="flex items-center gap-2 truncate">
          ...
          <span className="text-slate-200 font-semibold truncate">SURGE HUNTER PRO — DRIVER DASHBOARD</span>
          <span className="text-slate-600 hidden sm:inline">•</span>
          <span className="text-slate-400 hidden sm:inline">Singapore Earnings & Demand Monitor</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 bg-emerald-950/60 text-emerald-400 border border-emerald-800/40 px-2.5 py-0.5 rounded-full text-[11px] font-semibold">
            ...
            <span>{onlineUsers} Drivers Active</span>
          </div>
        </div>
      </div>
      ...
      <header className="border-b border-slate-800/80 bg-[#0F172A]/90 backdrop-blur-xl px-4 md:px-6 py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-lg md:text-xl font-bold tracking-tight text-white flex items-center gap-2">
            ...
            Surge Hunter
            <span className="text-xs bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 px-2 py-0.5 rounded-full font-semibold">Driver Edition</span>
          </h1>
          <p className="text-[11px] text-slate-400 mt-0.5">Real-time Surge Multipliers, Driver Clusters & Net Earnings Analytics</p>
        </div>

        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={() => setIsCalculatorOpen(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-600/30 transition-all shadow-md shadow-emerald-500/10"
          >
      ...
      <div className="bg-[#0D1322] border-b border-slate-800/80 px-4 py-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2.5 gap-1">
          <span className="text-xs font-bold text-slate-200 flex items-center gap-2">
            ...
            6-Hour Projected Surge Forecast: <span className="text-emerald-400 font-bold">{selectedHotspot.zoneName}</span>
          </span>
          <span className="text-[11px] text-slate-400">Select any cluster card below to update regional projection</span>
        </div>
        ...
        <div className="bg-[#0F172A]/80 backdrop-blur-md border border-slate-800/80 rounded-2xl p-4 flex flex-col gap-4 shadow-2xl">
          ...
          {userSurgeScore !== null && (
            <div className="bg-emerald-950/40 border border-emerald-800/40 p-3 rounded-xl text-xs">
              <span className="text-emerald-400 font-bold block text-[11px]">Your Current Location Surge Score:</span>
              ...
              return (
                <div 
                  ...
                  className={`bg-[#070A12] border rounded-xl p-3 transition-all cursor-pointer ${
                    isSelected ? 'border-emerald-500 bg-emerald-950/30 shadow-lg shadow-emerald-500/10' : 'border-slate-800/80 hover:border-slate-700'
                  }`}
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="font-semibold text-xs md:text-sm text-slate-100 flex items-center gap-1.5">
                        {spot.zoneName}
                        {isSelected && <span className="text-[9px] bg-emerald-600 text-white px-1.5 py-0.2 rounded-full font-bold">Active</span>}
                      </h3>
                      ...
                      <div className="flex items-center gap-2 mt-2 text-[11px] text-slate-300">
                        <span className="flex items-center text-emerald-400 font-semibold">
                          <DollarSign className="w-3.5 h-3.5" /> Gross Fare: S${estimatedGross}
                        </span>
                    ...
                    <div className="text-right">
                      <span className={`text-base md:text-lg font-black ${spot.score > 90 ? 'text-red-400' : spot.score > 80 ? 'text-amber-400' : 'text-emerald-400'}`}>
